...
### Boundary envelopes and platform responses

When consuming APIs that wrap responses in platform envelopes (such as ServiceNow Scripted REST `{ "result": T }`, GraphQL `{ "data": T }`, or JSON:API), unwrap and validate the envelope at the network/service boundary before returning data to view or domain layers.

- Do not leak platform envelopes into view components.
- Avoid ad-hoc casting in views (e.g. `(res as T & { result?: T }).result ?? res`). The adapter/service contract must guarantee clean unwrapping.
- Test adapters and services against both enveloped platform shapes and flat mock payloads.
- For ServiceNow Now SDK, server modules, and platform constraints, see [servicenow-now-sdk.md](./servicenow-now-sdk.md).
